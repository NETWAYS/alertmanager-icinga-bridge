...
type CLI struct {
	...
	Loglevel string `kong:"default='info',env='ALERTMANAGER_ICINGA_BRIDGE_LOGLEVEL',help='Loglevel'"`
	...
	DisplayNameAsServiceName bool              `kong:"default='false',env='ALERTMANAGER_ICINGA_BRIDGE_DISPLAY_NAME_AS_SERVICE_NAME',help='Leave display name as service name'"`
	...
	IcingaPassword           string            `kong:"required,env='ALERTMANAGER_ICINGA_BRIDGE_ICINGA_PASSWORD',help='Icinga Password'"`
	IcingaUser               string            `kong:"required,env='ALERTMANAGER_ICINGA_BRIDGE_ICINGA_USERNAME',help='Icinga Username'"`
	...
	// Garbage Collector
	GCInterval        time.Duration `kong:"default='15m',env='ALERTMANAGER_ICINGA_BRIDGE_GC_INTERVAL',help='Garbage collection interval for old alerts'"`
	HeartbeatInterval time.Duration `kong:"default='1m',env='ALERTMANAGER_ICINGA_BRIDGE_HEARTBEAT_INTERVAL',help='The heartbeat interval for the bridge self-monitoring service'"`
	...
	ListenAddr              string            `kong:"default='127.0.0.1:8888',env='ALERTMANAGER_ICINGA_BRIDGE_LISTEN_ADDR',help='Listening address for the incoming Alertmanager webhooks'"`
	...
	KeepFor                 time.Duration     `kong:"default='168h',env='ALERTMANAGER_ICINGA_BRIDGE_KEEP_FOR',help='How long to keep old alerts around after they have been resolved'"`
	StaticServiceVars       map[string]string `kong:"env='ALERTMANAGER_ICINGA_BRIDGE_STATIC_SERVICE_VAR',help='A variable to be set on each Icinga service (variable=value, can be repeated)'"`
}
